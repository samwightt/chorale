...
impl<'b, R, B: BlockRenderer<R>, I: InlineRenderer<R>, W: WrapperRenderer<R>>
    Renderer<'b, R, B, I, W>
{
    pub fn new(
        blocks: &'b BlockTableType,
    ...
    fn render_wrapper<'a>(&self, vector: &'a Vec<&'a BaseValueType>) -> R {
        ...
        match first.block {
            RootBlockType::BulletedList { properties: _ } => {
                self.wrapper_renderer.bulleted_list_wrapper(rendered)
            }
            RootBlockType::NumberedList { properties: _ } => {
                self.wrapper_renderer.numbered_list_wrapper(rendered)
            }
        ...
        let results = ids.iter().fold(acc, |(mut a, mut b), x| {
            ...
            if let Some(block) = element {
                if let Either::Left(result) = &block.value {
                    ...
                    if self.needs_grouping(&result.block)
                        && (self.can_be_grouped(&result.block, &b) || b.len() == 0)
                    {
                        b.push(result);
                        return (a, b);
        ...
        text.iter().fold(self.block_renderer.empty(), |acc, x| {
            if let Some(formatting) = &x.formatting {
                ...
                let resulting = formatting.iter().fold(initial, |other, y| {
                    return match y {
                        FormatType::NoContext(f) => match f {
            ...
            return self
                .wrapper_renderer
                .collect(vec![acc, self.inline_renderer.text(&x.text)]);
        })
    }

    pub fn render(&self, id: &str) -> R {
